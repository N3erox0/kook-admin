...
### 试错 8：旋转45° + 宽矩形裁剪方案（V2.13 ~ V2.13.2）

**方案**：装备左低右高斜放约45°，旋转45°将装备扶正，然后宽矩形裁剪(w:85%, h:55%)保留长条武器全长

**测试结果**（2026-05-15 实测）：
- 识别率未见明显提升，部分装备反而更差
- 第1格（剑）→ "玄秘法杖" 75% — **错误**
- 第4格 → "炎狱法杖" 66% — 置信度很低
- 旋转引入的额外灰色背景区域反而增加了 pHash 干扰
- 旋转后 resize 到 32×32 会引入更多插值模糊

**结论**：V2.13.2 回退旋转方案，改为纯不对称中心裁剪(15%/12%/70%/72%)。旋转方案理论上合理但实测效果不佳，原因可能是：
1. 渲染图中的装备并不统一斜放，有些是正向的
2. 旋转引入灰色填充区域增加了背景干扰
3. 截图分辨率较低，旋转后二次插值导致细节丢失

### 试错 9：pHash 不对称中心裁剪 + 灰128 + normalize（V2.13.2 当前）

**方案**：flatten(gray128) → 按模式遮罩角标 → 不对称中心裁剪(15%/12%/70%/72%) → 32×32 → 灰度 → normalize → DCT → pHash

**测试结果**（2026-05-15 实测）：
- 35 格全部识别出装备名（100% 填充率）
- 置信度范围 66%-81%，多数在 70-78%
- 部分装备识别错误（如剑→法杖），说明 pHash 对形状相似的不同装备区分度不够
- 数量识别基本准确

**结论**：pHash 方案已触及天花板。同类装备（武器类）之间的 pHash 差异太小，纯低频特征无法区分形状差异。需要引入 AI 特征向量方案（@xenova/transformers）提升精度到 95%+。

---

## 12. V2.14：@xenova/transformers 特征向量方案

### 12.1 方案概述

使用预训练视觉模型（ViT）提取 768 维特征向量，替代 64bit pHash：

```
pHash:    64bit 低频结构哈希 → 汉明距离 → 70-80% 准确率
特征向量:  768维 深度特征 → 余弦相似度 → 预期 95%+ 准确率
```

### 12.2 技术选型

- **包**：`@xenova/transformers`（Hugging Face 官方 JS 推理库）
- **模型**：`Xenova/vit-base-patch16-224`（ViT-Base，224×224 输入，768 维输出）
- **推理**：ONNX Runtime（包内自带，无需额外安装）
- **首次运行**：自动下载模型 ~350MB，之后缓存在本地
- **推理速度**：单张图 ~100-200ms（CPU）

### 12.3 预期架构

```
方案A（替代 pHash）：
  参考库：每个装备生成768维特征向量 → 存入DB
  识别时：截图切图 → 每格生成特征向量 → 余弦相似度匹配 → Top1

方案B（pHash粗筛 + 特征向量精排）：
  参考库：同时存 pHash + 特征向量
  识别时：pHash 粗筛 Top20 → 特征向量精排 → Top1
  好处：减少特征向量计算量（仅需对 Top20 候选计算）
```
