...
function isEquipment(name: string): boolean {
  ...
  return EQUIP_KW.some(kw => u.includes(kw));
}
...
async function main() {
  ...
  for (const r of rawData) {
    ...
    if (!name || !isEquipment(name)) continue;
  ...
  const values = items.map(item => {
    ...
    return `  ('${name}', '${item.uniqueName}', ${item.tier || 1}, 0, '${item.category}', ${item.tier || 1}, '${imageUrl}', '${desc}')`;
  });
